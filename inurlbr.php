...
function __bannerLogo() {
    ...
    return (!is_null($_SESSION['config']['no-banner']) ? NULL : system("") . "
...
function DB_Log_ext_command($com,$out){
  ...
  //var_dump($checkrow);
  if(!$checkrow){
    $db->query('INSERT INTO `ext-com-log`(`command`,`target`,`count`) VALUES ("'.htmlspecialchars($com).'","'.$out.'","'.$count.'");');  
  }else{
    $count = $count + $checkrow['count'];
    ...
    if (count($resultadoURL[0]) > 0) {
      ...
      echo $_SESSION['config']['dork'];
        ...
        foreach ($resultadoURL[0] as $url) {
          ...
          $sql = $db->ToDB('ip,www,raw_loot,script','"'.$ping.'","'.$cleanurl[0].'","|","inurlbr"','BigDump');
          //echo '\033[01;31m '.$sql.' \033[0m';
          $db->query($sql);
          //To DB K00B404 Mod           
          error_reporting(0);
